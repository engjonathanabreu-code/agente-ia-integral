...
const DEFAULT_TEMPLATE='contas_vencendo';
...
const daysUntil=v=>{const a=localDate(),b=String(v||'').slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(b))return 0;return Math.max(0,Math.round((Date.parse(`${b}T12:00:00Z`)-Date.parse(`${a}T12:00:00Z`))/86400000))};
...
function variablesFromTemplate(t){const raw=JSON.stringify(t?.components||t?.content||t||{});return [...new Set([...raw.matchAll(/\{\{\s*([\wÀ-ÿ]+)\s*\}\}/g)].map(m=>m[1]))]}
function templateBody(t,p,contact){
 ...
 const values={cliente:clientName,conta:p.accountName||'Conta cadastrada',dias:String(daysUntil(p.due)),valor:amount(p.value),vencimento:brDate(p.due)};
 const vars=variablesFromTemplate(t),named=/named/i.test(String(t.parameter_format||t.parameterFormat||''))||vars.some(v=>!/^\d+$/.test(v));
 let body;
 if(named){body={};for(const key of (vars.length?vars:['cliente','conta','dias','valor','vencimento']))body[key]=values[key]??values.conta;}
 else{body={'1':values.cliente,'2':values.conta,'3':values.dias,'4':values.valor,'5':values.vencimento};}
 const content=`Olá, ${values.cliente}. A conta ${values.conta} vence em ${values.dias} dia(s). Valor: R$ ${values.valor}. Vencimento: ${values.vencimento}.`;
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(req.body?.mode==='status')return res.status(200).json({ok:true,inbox:{id:ib.id,name:ib.name},configuredTemplate:configured,templates:list.map(t=>({name:t.name||t.template_name,status:t.status,category:t.category,language:t.language||t.language_code,parameterFormat:t.parameter_format||null,variables:variablesFromTemplate(t)}))});
