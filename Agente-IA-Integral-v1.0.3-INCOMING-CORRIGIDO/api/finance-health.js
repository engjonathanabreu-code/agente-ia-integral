...
export default async function handler(req,res){
  ...
  const canonicalUrl='https://ycdsyilyvaxslkwbkxyo.supabase.co';
  ...
  const out={ok:true,source:'financeiro_estado_modulos',configuredSupabaseHost:new URL(configuredUrl).host,canonicalSupabaseHost:new URL(canonicalUrl).host,supabaseServiceConfigured:!!key,chatwootConfigured:!!(chatBase&&account&&chatToken),cronTokenConfigured:!!(process.env.FINANCE_CRON_TOKEN||process.env.NUDGE_CRON_TOKEN||process.env.CRON_SECRET),whatsappTemplate:templateName,destinationConfigured:!!destination,destinationMasked:destination?`${String(destination).slice(0,4)}••••${String(destination).slice(-4)}`:null,tomorrow,canonicalStateAvailable:false,statePaymentCount:0,dueTomorrow:0,canonicalProbe:null,whatsappInbox:null,templateFound:false,templateStatus:null};
  if(key){
    const probe=async(base)=>{try{const r=await jsonFetch(`${base}/rest/v1/financeiro_estado_modulos?chave=in.(accountPayments,accountMasters)&select=chave,dados`,{apikey:key,Authorization:`Bearer ${key}`});const rows=r.ok?arr(r.data):[];const pay=rows.find(x=>x.chave==='accountPayments')?.dados||[];return{ok:r.ok,status:r.status,count:Array.isArray(pay)?pay.length:0,due:Array.isArray(pay)?pay.filter(p=>String(p?.due||'').slice(0,10)===tomorrow&&!/paga|pago|cancelad/i.test(String(p?.status||''))).length:0};}catch(e){return{ok:false,error:String(e?.message||e)}}};
    const configured=await probe(configuredUrl);out.canonicalStateAvailable=configured.ok;out.statePaymentCount=configured.count||0;out.dueTomorrow=configured.due||0;
